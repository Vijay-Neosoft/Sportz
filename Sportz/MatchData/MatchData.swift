...
//
//   let matchDetailResponse = try? JSONDecoder().decode(MatchDetailResponse.self, from: jsonData)
...
class Team: Codable {
    ...
    required init(from decoder: Decoder) throws {
        ...
        nameFull = try values.decodeIfPresent(String.self, forKey: .nameFull)
        nameShort = try values.decodeIfPresent(String.self, forKey: .nameShort)
        players = try values.decodeIfPresent([String: Player].self, forKey: .players)
        playersList = players?.values.map({ return $0 }) ?? [Player]()
            }
